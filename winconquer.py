...
# Function to fetch win odds
def fetch_win_odds(race_no):
    ...
    race_odds_list = json_data.get("data", {}).get("win", {}).get("raceOddsList", [])
    odds = {}
...
# Streamlit layout
st.title("🏇 HKJC Win (獨贏) Odds & Dutching Calculator")
...
# Fetch odds
odds = fetch_win_odds(race_no)

if not odds:
    st.warning("No Win odds available.")
else:
    st.subheader(f"Race {race_no} - Win Odds (獨贏)")
    df = pd.DataFrame(list(odds.items()), columns=["Horse No", "Win Odd"])
    st.dataframe(df)
    ...
    if selected_horses:
        ...
        st.subheader("💰 Dustching Allocation")
